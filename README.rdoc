...
Sinatraにおいて、ルートはHTTPメソッドと対になったURLにマッチするパターンです
...
リクエストされたルートは定義されたルートパターンによってマッチングを行い、
最初にマッチしたものが実行されます

ルートパターンは名前付きパラメータを持つことができ、それらには
...
ルートパターンはsplat(ワイルドカード)パラメータを持つことができ、
